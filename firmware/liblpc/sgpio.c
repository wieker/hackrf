...
// SGPIO pin 0 outputs slice A bit 0.
    ((LPC_SGPIO_T *)LPC_SGPIO)->SLICE_MUX_CFG[0] =
            (1L <<  2);
...
// Slice A
    ...
    LPC_SGPIO->COUNT[0] = 0;
    LPC_SGPIO->POS[0] = 32;
    LPC_SGPIO->REG[0] = 0x00;     // Primary output data register
    LPC_SGPIO->REG_SS[0] = 0x00;  // Shadow output data register

// Start SGPIO operation by enabling slice clocks.
    LPC_SGPIO->CTRL_ENABLED =
            (1L <<  0);     // Slice A
    LPC_SGPIO->SET_EN_1 = 0x01;
...
void sgpio_isr_custom() {
    ...
    "str r0, [%[p], #0]\n\t"
    :
    : [SGPIO_REG_SS] "l" (LPC_SGPIO_BASE + 0x100),
    [p] "l" (p)
    : "r0"
    );
